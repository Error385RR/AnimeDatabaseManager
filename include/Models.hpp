...
namespace anime{
    ...
    inline void from_json(const json& j, Anime& anime){
        ...
        anime.synopsis = j.value("synopsis", std::string{});//.get<std::string>();
        anime.source = j.value("source", std::string{});
        anime.status = j.value("status", std::string{});
        anime.mediaType = j.at("media_type").get<std::string>();
        anime.startDate = j.value("start_date", std::string{});
        anime.endDate = j.value("end_date", std::string{});
        ...
        anime.nsfw = j.value("nsfw", std::string{});
        anime.averageEpisodeDurationSec = j.value("average_episode_duration", 0);
        anime.popularity = j.value("popularity", 0);
        anime.mean = j.value("mean", 0.0);
        anime.numEpisodes = j.value("num_episodes", 0);
        anime.startSeason = j.value("start_season", start_season{});
        anime.studios = j.value("studios", std::vector<Studio>{});
        anime.broadcasts = j.value("broadcast", broadcast{});
        anime.genres = j.value("genres", std::vector<Genre>{});
        anime.thumbnail = j.value("main_picture", Image{});
        anime.pictures = j.value("pictures", std::vector<Image>{});
