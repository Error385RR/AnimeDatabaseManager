...
public:
    ...
    httphandler(){
        CURLcode curlglobalinitcode = curl_global_init(CURL_GLOBAL_DEFAULT);
        if(curlglobalinitcode != CURLE_OK){
            throw std::runtime_error("Could Not Initialize The Curl Library: " + std::string(curl_easy_strerror(curlglobalinitcode)));
        }
    }
    ...
    static HttpResponse get(const std::string& url, std::vector<HttpHeader>& headers){
        return perform(url, headers, HttpMethod::GET);
    };

    static HttpResponse post(const std::string& url, const std::vector<HttpHeader>& headers, const std::string& body){
